...
import (
	...
	"github.com/devmiro/go-chat/config"
	"github.com/devmiro/go-chat/http/middlewares"
	"github.com/devmiro/go-chat/http/routes"
	"github.com/devmiro/go-chat/models"
...
func run() error {
	...
	if fileLogging == "true" {
		file, err := os.Create(fmt.Sprintf("./applog-%s.txt", time.Now().Format(time.DateOnly)))
		if err != nil {
